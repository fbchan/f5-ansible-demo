# This is a script written to manage BIG-IP license activation for BIG-IQ License Manager
...
parser.add_argument('--regkey', help='Registration Key string. By default retrieve available license string from LM. This to overwrite', required=False)
...
def lm_unused_regpool(auth_token,ip,rpoolname):
	...
	for regkey in json_data2['items']:
		...
		key = regkey['regKey']
		url3 = 'https://'+ip+'/mgmt/cm/device/licensing/pool/regkey/licenses/'+id+'/offerings/'+key+'/members/'
		#url3 = 'https://'+ip+'/mgmt/cm/device/licensing/pool/regkey/licenses/'+id+'/offerings/EZAZF-JYCQJ-KEIFE-OQWCE-QDNEEQY/members/' 
		resp3 = requests.get(url3, headers=headers, verify=False)
		json_data3 = json.loads(resp3.text)
		#print len(json_data3['items'])
...
def lm_assign_regkey(auth_token,ip,bigip_ip,username,password,regkeypool_id,unused_regkey):
	...
	if resp.status_code != 200:
		...
		print "-------------------------------------------------------------"
		print "License key " + unused_regkey + "successfuly assigned"
		print "-------------------------------------------------------------"
		print(json.dumps(resp.json(), indent=2))
	return;
